  ...
  properties: {
    ...
    preferenceConstraints: {
      type: "ARRAY", nullable: true, items: {
        type: "OBJECT",
        properties: {
          mode: { type: "STRING" }, targetType: { type: "STRING" }, target: { type: "STRING" },
          placeIds: { type: "ARRAY", items: { type: "STRING" }, nullable: true },
          periods: { type: "ARRAY", items: { type: "STRING" }, nullable: true },
          days: { type: "ARRAY", items: { type: "INTEGER" }, nullable: true },
          strength: { type: "STRING", nullable: true },
          confidence: { type: "NUMBER", nullable: true }
        },
        required: ["mode", "targetType", "target"]
      }
    },
    timingPreference: {
      type: "OBJECT",
...
- preferenceConstraints converts natural-language preferences into composable structured constraints. It is not a closed vocabulary: infer meaning from context.
  - mode: avoid, prefer, or require.
  - targetType: category or place. For places use only IDs from the supplied catalog.
  - target may be water, temple, boat, island, hiking, active, cultural, cafe, viewpoint, beach, nature, shopping, food, full_day, land, sea, or any.
  - periods may contain morning, afternoon, evening, or any; combine multiple periods when the user says “เช้าเย็น”, “ทั้งเช้าและเย็น”, etc.
  - days may contain multiple day numbers; an empty array means every day. Interpret “ทั้ง 2 วัน/ทั้ง 4 วัน/ทุกวัน” as all days in the trip and “วันที่ 1 และ 4” as [1,4].
  - strength=hard for explicit prohibitions/requirements and soft for preferences.
  - If the request is genuinely ambiguous, leave preferenceConstraints empty rather than guessing.
...
`;
    ...
    const safe = {
      ...
      preferenceConstraints: Array.isArray(result.preferenceConstraints) ? result.preferenceConstraints.slice(0,20).map(pref => ({
        mode: ["avoid","prefer","require"].includes(pref?.mode) ? pref.mode : null,
        targetType: pref?.targetType === "place" ? "place" : "category",
        target: cleanString(pref?.target, 40).toLowerCase(),
        placeIds: Array.isArray(pref?.placeIds) ? pref.placeIds.filter(id => placeIds.has(id)).slice(0,30) : [],
        periods: Array.isArray(pref?.periods) ? [...new Set(pref.periods.map(v=>String(v).toLowerCase()).filter(v=>["morning","afternoon","evening","any"].includes(v)))] : ["any"],
        days: Array.isArray(pref?.days) ? [...new Set(pref.days.map(Number).filter(v=>Number.isInteger(v)&&v>=1&&v<=14))].sort((a,b)=>a-b) : [],
        strength: pref?.strength === "hard" ? "hard" : "soft",
        confidence: Number.isFinite(Number(pref?.confidence)) ? Math.max(0,Math.min(1,Number(pref.confidence))) : 0.85
      })).filter(p => p.mode && p.target && (p.targetType !== "place" || p.placeIds.length)),
      timingPreference: (() => {
