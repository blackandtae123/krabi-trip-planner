  ...
  properties: {
    ...
    timingPreference: {
      ...
      properties: {
        ...
        avoidWaterAfternoon: { type: "BOOLEAN", nullable: true }
      }
    }
  },
...
- timingPreference is ONLY about how the traveler wants their day paced — it does NOT decide real opening/closing hours or travel times, those come from the app's own schedule engine, not from you.
  ...
  - avoidWaterAfternoon: true ONLY when the traveler explicitly says they do not want swimming/water activities in the afternoon (for example “ไม่อยากเล่นน้ำช่วงบ่าย”, “avoid swimming in the afternoon”). Otherwise false/null. This is a scheduling preference, not a factual claim about a place.
...
- Interpret natural language semantically even when wording is unfamiliar, slang, shorthand, or misspelled. For a clearly expressed durable preference, also return learnedPreferences with key/value/confidence >= 0.80. Normalize “บ่ายไม่เอาเปียก”, “ช่วงบ่ายไม่ลงน้ำ”, “ไม่อยากเล่นน้ำหลังเที่ยง” to avoid_water_afternoon=true.
- Do not learn ambiguous statements.
`;
    ...
    const safe = {
      ...
      learnedPreferences: (Array.isArray(result.learnedPreferences) ? result.learnedPreferences : []).slice(0,12).map(item => ({
        key: cleanString(item?.key, 80),
        value: cleanString(item?.value, 240),
        confidence: Math.max(0, Math.min(1, Number(item?.confidence) || 0)),
        scope: item?.scope === "day" ? "day" : "trip",
        day: Number.isInteger(item?.day) ? item.day : null
      })).filter(item => item.key && item.value && item.confidence >= 0.80),
      timingPreference: (() => {
        ...
        const avoidWaterAfternoon = tp.avoidWaterAfternoon === true;
        if (!preferredStartTime && !mustReturnBy && !extraBufferMinutes && !avoidWaterAfternoon) return null;
        return { preferredStartTime, mustReturnBy, extraBufferMinutes, avoidWaterAfternoon };
      })()
    };
